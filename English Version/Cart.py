...
layout = [
    [sg.Text("Cart", justification='center', expand_x=True)],
    [sg.Text(key="linha_1", size=(50, 4), background_color="White", text_color = "Black"), sg.Button("Delete", key="Deletar_1")],
    [sg.Text(key="linha_2", size=(50, 4), background_color="White", text_color = "Black"), sg.Button("Delete", key="Deletar_2")],
    [sg.Text(key="linha_3", size=(50, 4), background_color="White", text_color = "Black"), sg.Button("Delete", key="Deletar_3")],
    [sg.Text(key="linha_4", size=(50, 4), background_color="White", text_color = "Black"), sg.Button("Delete", key="Deletar_4")],
    [sg.Text("Total:", size=(5, 1)), sg.Text("$"), sg.Text("0", key="total")],
    [sg.Text("Quantity of items:", size=(18, 1)), sg.Text("0", key="total de produtos", size=(30, 1))],
    [sg.Button("Previous page"),sg.Text("Page:"), sg.Text("1", key="pagina"), sg.Button("Next page")],
    [sg.Text()],
    [sg.Text("Insert item")],
    [sg.Text("Name:", size=(5, 1)), sg.Input(key="Produto")],
    [sg.Text("Price:", size=(5, 1)), sg.Input(key="Valor")],
    [sg.Text("How many?"), sg.Button("-"), sg.Input(key="Quantidade", size=(10, 1), justification='center', default_text="1"), sg.Button("+")],
...
def exibir_lista(pagina):
    ...
    try:
        ...
        valor_1 = f"""Name: {lista_atual[0][0]} 
Price: ${lista_atual[0][1]}  
Quantity: {lista_atual[0][2]}
Total: ${total_1}"""
    ...
    try:
        ...
        valor_2 = f"""Name: {lista_atual[1][0]} 
Price: ${lista_atual[1][1]} 
Quantity: {lista_atual[1][2]}
Total: ${total_2} """
    ...
    try:
        ...
        valor_3 = f"""Name: {lista_atual[2][0]} 
Price: ${lista_atual[2][1]}  
Quantity: {lista_atual[2][2]}
Total: ${total_3}"""
    ...
    try:
        ...
        valor_4 = f"""Name: {lista_atual[3][0]} 
Price: ${lista_atual[3][1]}
Quantity: {lista_atual[3][2]}
Total: ${total_4}"""
...
def excluir_item(numero):
    ...
    if not item_removido:
        ...
        window["feedback"].update(f"This item was deleted: {item_removido}")
...
#Evento que acontece quando se adiciona um item.
    if event == "Add to cart":
        ...
        if not valores_completos or not valores_convertidos:
            window["feedback"].update("It was not possible to add this item. Try something different.")
        else:
            window["feedback"].update("Item added.")
...
#Evento que acontece quando se pressiona o botão de adicionar quantidade
    if event == "+":
        ...
        if not quantidade_convertida:
            window["feedback"].update("This is not a valid quantity.") 
        else:
